namespace Numbers
{
    ...
    public class PawnColumnWorker_JobQueued : PawnColumnWorker_Text
    {
        ...
        public override int Compare(Pawn a, Pawn b) => (a.jobs?.jobQueue?.Count ?? 0).CompareTo(b.jobs?.jobQueue?.Count ?? 0);

        protected override string GetTip(Pawn pawn) => pawn.jobs?.jobQueue?.Count.ToString();

        public override int GetMinWidth(PawnTable table) => Mathf.Max(base.GetMinWidth(table), 200);

        public override int GetMinHeaderHeight(PawnTable table) => Mathf.CeilToInt(Text.CalcSize(Numbers_Utility.WordWrapAt(this.def.LabelCap, this.GetMinWidth(table))).y);
    }
}
